...
var EntityName, PGConnect, TYPE, User, addUser, appCrypto, crypto, debug, deleteFromDatabaseUserForEmail, deleteQuery, emailClient, getForEmail, getSaltForEmail, globals, insertIntoDatabase, insertQuery, removeUser, reset, resetPasswordForUserWithEmail, setPasswordForUserWithEmail, updatePasswordInDatabase, updatePasswordQuery, verifyCredentials;
...
getForEmail = function(email, callback) {
  return PGConnect(function(err, client, done) {
    ...
    return client.query({
      ...
      if (result.rows[0]) {
        return typeof callback === "function" ? callback(null, new User(result.rows[0].email)) : void 0;
      } else {
...
resetPasswordForUserWithEmail = function(userEmail, client, callback) {
  debug("Resetting password for email: " + userEmail);
  return reset.addResetKeyForEmail(userEmail, null, client, callback);
};

updatePasswordQuery = function(values, client, done, callback) {
  return client.query({
...
updatePasswordInDatabase = function(email, password, client, callback) {
  if (typeof client === "function") {
    callback = client;
    client = void 0;
  }
...
setPasswordForUserWithEmail = function(email, newPassword, client, callback) {
  return updatePasswordInDatabase(email, newPassword, client, callback);
};
