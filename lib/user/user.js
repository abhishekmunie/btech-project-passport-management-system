...
var EntityName, PGConnect, TYPE, User, addUser, appCrypto, crypto, debug, deleteFromDatabaseUserForEmail, deleteQuery, email, getSaltForEmail, globals, removeUser, resetPasswordForUserWithEmail, verifyCredentials;
...
User = (function() {
  ...
  User.prototype.insertIntoDatabase = function(password, client, callback) {
    if (typeof client === "function") {
      callback = client;
      client = void 0;
    }
    crypto.randomBytes(126, (function(_this) {
      return function(ex, buf) {
        var salt;
        if (ex) {
        ...
        appCrypto.genKey(password, salt, function(err, key) {
          ...
          if (client != null) {
            insertQuery([_this.email, key, salt], client, null, callback);
          } else {
            PGConnect(function(err, client, done) {
              ...
              insertQuery([_this.email, key, salt], client, done, callback);
            });
          }
        });
      };
...
addUser = function(email, password, client, callback) {
  ...
  return new User(email).insertIntoDatabase(password, client, callback);
};
