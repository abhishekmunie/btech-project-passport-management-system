...
router.post('/va-management', function(req, res, next) {
  ...
  switch (action) {
    case 'setValidationAuthorityEmail':
      ...
      pgo.getRegionIdForPGOWithEmail(req.session.user.email, function(err, Id) {
        if (err) {
        ...
        return va.addVAForRegionId(VAEmail, Id, function(err, row) {
          ...
          res.locals.success = {
            message: "validation Authority's Email was successfully Authorize."
          };
          next();
        });
      });
    ...
    case 'unsetValidationAuthorityEmail':
      ...
      region.unsetPGOForRegionId(Id, function(err, row) {
        ...
        console.log(row);
        res.locals.success = {
          message: "validation Authority's Email was successfully Unauthorize."
        };
        next();
      });
      break;
...
router.all('/va-management', function(req, res, next) {
  pgo.getValidationAuthorititesUnderPGOWithEmail(req.session.user.email, function(err, vas) {
    if (err) {
      return next(err);
    }
